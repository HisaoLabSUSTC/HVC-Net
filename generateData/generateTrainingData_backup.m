...
% -----------task-------------------
    ...
    for i=num_on_triPF+num_on_invtriPF+1:num_on_triPF+num_on_invtriPF+num_on_random
        ...
        while check_num_points < num
            ...
            solutions = rand(50*data_num,M);
            % get non-dominated solutions
            [FrontNo,MaxFNo] = NDSort(solutions,50*data_num);
            ...
            disp(['check_num_points=', num2str(check_num_points)])
        end
        ...
        elseif strcmp(mode, 'best')
            for front_idx = 1:MaxFNo
                selectedNo = front_idx;
                ndsolutions = solutions(FrontNo==selectedNo,:); 
                if num > size(ndsolutions, 1)
                    continue
                end
            end
